...
define('PATH_DB', $dir_path . 'etc' . DIRECTORY_SEPARATOR);

require PATH_DB . 'pdoconn.php';

$sessData = $_POST['sessionVariable'];
$sessPrpty = $_POST['sessionPrpty'];
...
if(isset($_POST["search"]["value"]))
{
    ...
    AND u_landlord="'.$sessData.'" AND ApartmtName="'.$sessPrpty.'"';
...
function count_all_data($conn)
{
 global $sessData;
 global $sessPrpty;
 $query = "SELECT * FROM hseUnits WHERE u_landlord='$sessData' AND ApartmtName='$sessPrpty'";
 $statement = $conn->prepare($query);
 $statement->execute();
